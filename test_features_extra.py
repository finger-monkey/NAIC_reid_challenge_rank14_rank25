...
def get_image(filename, model):
    ...
    img_flip = transforms.RandomHorizontalFlip(p=1.0)(img)
    ...
    img_flip = transforms.ToTensor()(img_flip)
    img_flip = transforms.Normalize(mean, std)(img_flip)
    img_flip = img_flip.unsqueeze(0)
    img_flip = img_flip.cuda()

    feature = model(img)
    feature_flip = model(img_flip)

    feature = torch.nn.functional.normalize(feature, dim=1, p=2)
    feature_flip = torch.nn.functional.normalize(feature_flip, dim=1, p=2)

    feature = torch.cat((feature, feature_flip), 1)
    feature = torch.nn.functional.normalize(feature, dim=1, p=2)
    return feature
